...
namespace MyFinanceServer.IntegrationTests
{
	...
	public class CategoryTest : ControllerTestBase
	{
		...
		{
			var request = CreateAuthJsonRequest("POST", "/api/v1/categories/", new Api.Dto.Category() { Title = "Category #1" });
			var response = await SendRequest(request);
		...
		public async Task UpdateRule_ShouldReturnContent()
		{
			...
			var request = CreateAuthJsonRequest("PUT", "/api/v1/categories/" + category.Id, new Api.Dto.Category
			{
				Id = category.Id,
				Title = "Category #2"
			});
