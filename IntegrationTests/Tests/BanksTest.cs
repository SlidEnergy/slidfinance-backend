...
namespace MyFinanceServer.IntegrationTests
{
	...
	public class BanksTest : ControllerTestBase
	{
		...
		{
			var request = CreateAuthJsonRequest("POST", "/api/v1/banks/", new Api.Dto.Bank () {  Title = "Bank #1" });
			var response = await SendRequest(request);
		...
		public async Task UpdateBank_ShouldReturnContent()
		{
			...
			var request = CreateAuthJsonRequest("PUT", "/api/v1/banks/" + bank.Id, new Api.Dto.Bank
			{
				Id = bank.Id,
				Title = "Bank #2"
			});
