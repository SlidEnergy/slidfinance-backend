...
namespace SlidFinance.WebApi.UnitTests
{
	public class ApiImportControllerTests : TestsBase
    {
		...
		[Test]
		public async Task ImportWithNewMerchant_ShouldCallAddMccMethod()
		{
			var bank = new Bank() { Title = "Bank #1" };
			_db.Banks.Add(bank);
			var account = await _db.CreateAccount(_user);
			...
			_db.Mcc.Add(mcc);
			await _db.SaveChangesAsync();
			var merchant = new Models.Merchant() { MccId = mcc.Id, Name = "Description #1" };
			...
			_merchantService.Verify(x => x.AddAsync(It.Is<Models.Merchant>(m => m.MccId == mcc.Id && m.Name == transaction1.Description)));
		}

		[Test]
		public async Task ImportWithExistMerchant_ShouldCallAddMccMethod()
		{
			var bank = new Bank() { Title = "Bank #1" };
			_db.Banks.Add(bank);
			var account = await _db.CreateAccount(_user);
			...
			_mccService.Setup(x => x.GetListAsync()).ReturnsAsync(new List<Mcc>());
			_mccService.Setup(x => x.AddAsync(It.IsAny<Mcc>())).ReturnsAsync(mcc);
			_merchantService.Setup(x => x.GetListAsync()).ReturnsAsync(new List<Models.Merchant>() { merchant });
			_merchantService.Setup(x => x.AddAsync(It.IsAny<Models.Merchant>())).ReturnsAsync((Models.Merchant x) => x);

			var result = await _service.Import(_user.Id, new PatchAccountDataBindingModel() { Code = account.Code, Balance = 100, Transactions = new Dto.Transaction[] { transaction1 } });

			_merchantService.Verify(x => x.AddAsync(It.IsAny<Models.Merchant>()), Times.Never);
		}
	}
}
