...
namespace SlidFinance.WebApi.UnitTests
{
	public class AccountControllerTests : TestsBase
    {
        ...
        public async Task GetAccounts_ShouldReturnList()
        {
			...
			_service.Setup(x => x.GetListWithAccessCheckAsync(It.IsAny<string>(), It.IsAny<int?>())).ReturnsAsync(new List<BankAccount>() { account1, account2 });

            var result = await _controller.GetList();

            Assert.AreEqual(2, result.Value.Count());

			_service.Verify(x => x.GetListWithAccessCheckAsync(It.Is<string>(u => u == _user.Id), It.Is<int?>(b => b == null)));
		}
    }
}